...
const AddRecipePage = () => {
  ...
  const [formData, setFormData] = useState({
    name: "", category: "", cuisine: "", difficulty: "", prepTime: "", ingredients: "", instructions: "", url: "",
  });
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("Submitting to Database:", recipePayload);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0f121e] p-4 md:p-8">
      ...
      >
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Add New Recipe</h1>
        <p className="text-gray-500 dark:text-gray-400 mb-8">Fill in the details below to share your creation.</p>

        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          
          <div className="md:col-span-2">
            <label className={labelClass}>Recipe Image Link <span className="text-red-500">*</span></label>
            <input required type="url" placeholder="https://example.com/image.jpg" onChange={(e) => setFormData({...formData, url: e.target.value})} className={inputClass} />
          </div>

          <div className="md:col-span-2">
            <label className={labelClass}>Recipe Name <span className="text-red-500">*</span></label>
            <input required placeholder="Enter recipe name" onChange={(e) => setFormData({...formData, name: e.target.value})} className={inputClass} />
          </div>

          <div>
            <label className={labelClass}>Category <span className="text-red-500">*</span></label>
            <select required onChange={(e) => setFormData({...formData, category: e.target.value})} className={inputClass}>
          ...
          <div>
            ...
            <select required onChange={(e) => setFormData({...formData, cuisine: e.target.value})} className={inputClass}>
          ...
          <div>
            ...
            <select required onChange={(e) => setFormData({...formData, difficulty: e.target.value})} className={inputClass}>
          ...
          <div>
            ...
            <input required placeholder="e.g., 30 mins" onChange={(e) => setFormData({...formData, prepTime: e.target.value})} className={inputClass} />
          </div>

          <div className="md:col-span-2">
            <label className={labelClass}>Ingredients <span className="text-red-500">*</span></label>
            <textarea required rows="4" placeholder="List your ingredients here..." onChange={(e) => setFormData({...formData, ingredients: e.target.value})} className={inputClass}></textarea>
          </div>

          <div className="md:col-span-2">
            <label className={labelClass}>Instructions <span className="text-red-500">*</span></label>
            <textarea required rows="4" placeholder="Step-by-step instructions..." onChange={(e) => setFormData({...formData, instructions: e.target.value})} className={inputClass}></textarea>
          </div>
